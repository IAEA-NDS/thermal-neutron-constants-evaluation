"""Benchmark: Reproduce Axton 1986 Table 8 (Maxwellian data omitted).

Runs the iterative GLS fit using only microscopic (non-Maxwellian)
measurements and compares with Table 8 of the Axton 1986 report.

Expected results from the report:
  chi2 = 48, 101 measurements, 38 parameters, 63 dof

Note: Our classification yields 98 microscopic measurements vs the
report's 101. The 3-measurement discrepancy is likely due to borderline
cases (g-factor or Westcott factor measurements). This should be
investigated further against the earlier Axton papers (1) and (2).
"""
...
# Maxwellian DERIVED functions: these combine cross sections with
# g-factors (FA=ABS*WGA, FF=FIS*WGF, CAP=FA-FF, FFH=FF*HLF, etc.)
# Standalone WGA, WGF, GC116, GA116 measurements are NOT excluded
# since they are theoretical/compilation values, not reactor experiments.
# FH1 = FIS*HLF is monoenergetic (no g-factor involved).
...
def is_maxwellian(expr):
    """Check if a measurement expression involves Maxwellian-derived quantities."""
...
def run_benchmark():
    """Run the microscopic benchmark and print comparison with Table 8."""
    data_path = str(
    ...
    # Build parameter space — remove CAP 34 which is unconstrained
    # without Maxwellian data (measurement 5 excluded)
    ...
    # Print results
    print("=" * 78)
    print("BENCHMARK: Axton 1986 Table 8 — Maxwellian data omitted")
    print("=" * 78)
    ...
    print(f"{'N measurements':<25s} {len(measured):12d} {REPORT_N_MEAS:12d}"
          f"{'':>8s}" if len(measured) == REPORT_N_MEAS else
          f"{'N measurements':<25s} {len(measured):12d} {REPORT_N_MEAS:12d} {'DIFF':>8s}")
    ...
    print(f"\nNote: {len(measured)} measurements used vs {REPORT_N_MEAS} in report.")
    print("The 3-measurement discrepancy needs investigation against earlier papers.")
